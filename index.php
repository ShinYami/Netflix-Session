	...
	$req->execute(array($email));
		...
		while($email_verification = $req->fetch()){
			if($email_verification['numberEmail'] != 1) {
				header('location: index.php?error=1&message=Erreur Authentification.');
				exit();
			}
		}

		$req = $dbb->prepare("SELECT * FROM user WHERE email = ?");
		$req->execute(array($email));
		...
		while($user = $req->fetch()){
			if($password == $user['password']) {
				...
				if(isset($_POST['auto'])) {
					setcookie('auth', $user['secret'], time() + 364*24*3600, '/', null, true, false);
				}
